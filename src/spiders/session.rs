...
impl SessionManager {
    ...
    pub fn ensure_default(&mut self) {
        if !self.sessions.contains_key("default") {
            self.sessions.insert("default".to_string(), Fetcher::new(self.default_config.clone()));
        }
    }
    ...
    pub async fn fetch(&self, request: &SpiderRequest) -> Result<Response, String> {
        let session_id = if request.session_id().is_empty() { "default" } else { request.session_id() };
        let fetcher = self.sessions.get(session_id).ok_or_else(|| format!("Session '{}' not found", session_id))?;

        match request.method() {
            "GET" => fetcher.get(request.url()).await.map_err(|e| e.to_string()),
            "POST" => fetcher.post(request.url(), request.body(), None).await.map_err(|e| e.to_string()),
            "PUT" => fetcher.put(request.url(), request.body(), None).await.map_err(|e| e.to_string()),
            "DELETE" => fetcher.delete(request.url()).await.map_err(|e| e.to_string()),
