...
fn test_css_selector_for_li_includes_nth_of_type() {
    ...
    assert!(css.contains("nth-of-type"), "Expected 'nth-of-type' in: {}", css);
}
...
fn test_xpath_selector_with_id() {
    ...
    assert!(xpath.contains("@id='main'"), "Expected \"@id='main'\" in: {}", xpath);
}
...
fn test_full_css_selector_includes_body() {
    ...
    assert!(css.contains("body"), "Expected 'body' in full path: {}", css);
}
